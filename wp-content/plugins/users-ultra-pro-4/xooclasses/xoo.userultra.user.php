		...
		if( !isset($_GET['gal_id']) && !isset($_GET['photo_id']) && !isset($_GET['my_galleries'])  && !isset($_GET['my_followers'] )  && !isset($_GET['my_posts']) && !isset($_GET['my_following'])  && !isset($_GET['my_friends'])  && !isset($_GET['my_videos']) && !isset($_GET['my_topics'])) 
		{
		  ...
		  if(in_array(1, $cols))
		  {
			  ...
			  if(count($cols)==1 || count($cols)==2 || count($cols)==3)
		  	  {			  
				  $html .='<!------ Column 1 -----><div class="col-uultra-1" '.$dimension_style.'>
					<ul>'. $xoouserultra->customizer->get_profile_col_widgets($user_id,1, $cols, $atts).'</ul></div>';	
		  ...
		  if(in_array(2, $cols))
		  {
			   //det lenght			   
			    if(count($cols)==2 || count($cols)==3)
				  ...
				  $html .='<!------ Column 2 -----><div class="col-uultra-2" '.$dimension_style.'><ul>'.$xoouserultra->customizer->get_profile_col_widgets($user_id,2,$cols, $atts).'</ul></div>';
		  ...
		  if(in_array(3, $cols))
		  {
			   //det lenght			   
			    if(count($cols)==3)
				  ...
				  $html .='<!------ Column 3 -----><div class="col-uultra-3" '.$dimension_style.'><ul>
					 '. $xoouserultra->customizer->get_profile_col_widgets($user_id,3,$cols, $atts).'</ul>     
				  </div>  ';
		 ...
		 $html .=  ' <script type="text/javascript" > xoo_load_wallmessags("'.$user_id.'" );</script>';
			...
			$html .= $xoouserultra->photogallery->reload_galleries_public($user_id, $gallery_type);
